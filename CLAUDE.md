...
- **Dashboard (`components/DashboardView.tsx`):** the single-focus home, distilled from `loadCalendarData` (no new data). A `max-w-5xl` two-zone layout (stacks &lt;`lg`): primary = the **Focus module** (violet `accent-soft` card — the #1 priority big, with deterministic reason-chips + a one-line AI coach + Open/See-my-plan, and a "Next up" mini) over a **priority-ranked Today list** (urgency by color, **type by icon+label, NOT course color**, ~48px rows); rail = **This week** ("Xh of work · Yh free"), **Overdue** list, jump links. Per the June UX pass: greeting demoted to one quiet line, small text on `text-muted` (not `faint`) for AA, no course-color bars on this page. Only links into Calendar/Timeline — keep it low-density (no grids/controls/filters). Shares the once-per-session auto-sync key (`sp_autosynced`) with the Calendar.
