...
- **Calendar & Timeline views:** the student's coursework, replacing the old Plan page. `lib/calendarData.ts` (`loadCalendarData`) is the shared loader — it reuses the scheduler, the priority ranking, busy-hours, and stored events to produce one payload. **Calendar** (`/calendar`, home): Day (default) / Week / Month, each with a study-coach summary + Canvas items placed on due dates + Google "busy" blocks. **Timeline** (`/timeline`): a Gantt — courses as rows, the scheduler's work projected across the week in recommended order, with a Past-Due lane. Both carry the folded-in safety features (at-risk **AttentionBanner**, **RecommendedOrder**, undated/completed). Per-course colors come from `lib/courseColor.ts` (the one sanctioned non-token color); item TYPE from `lib/itemType.ts` (needs `Assignment.submissionType`, synced from Canvas `submission_types`).
- **AI study coach:** `lib/briefing.ts` `generatePeriodBriefing` + `/api/calendar/briefing?view=&start=&days=` writes a learning-science "game plan" for the selected period (spaced practice, retrieval, interleaving, focus blocks). Advisory only — it narrates the already-ranked, deadline-safe schedule; **never reorders or invents deadlines**. Fails open (renders nothing without `GEMINI_API_KEY`). Admin-editable at `/admin/ai` (`PERIOD_COACH_PROMPT_KEY`).
...
- **AI assignment analysis:** `lib/analysis.ts` + `lib/analysisStore.ts` + `/api/analyze` (lazy, batched, cached on the Assignment row via a content hash) have Gemini estimate each assignment's **effort** (hours + quick/medium/long → feeds the scheduler per-assignment instead of the flat default) and a **one-line summary** (shown in item details). Fails open; its prompt is an editor on `/admin/ai`.
