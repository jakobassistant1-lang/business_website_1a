...
- **Dashboard (`components/DashboardView.tsx`):** the calm landing — a single `max-w-2xl` column distilled from `loadCalendarData` (no new data): greeting + one-line AI coach (`CoachLine`, fails open), overdue strip (`AttentionBanner`), the ONE top priority (`recommendations[0]` → `ItemDetail`), a capped "Today" list, and a 3-stat "this week" pulse. It only links into Calendar/Timeline for detail — keep it low-density (no grids/controls/filters here). Shares the once-per-session auto-sync key (`sp_autosynced`) with the Calendar.
