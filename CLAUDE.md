...
- **Study sessions:** `itemType` exam/quiz items get `studyLeadDays` (from `User.studyDaysTest` / `studyDaysQuiz`, set in Settings). `generatePlan` schedules their effort as **study blocks** (`DayBlock.study`) only within `[due − leadDays, due]`. The **planning window is fixed at 7 days** (`PLAN_WINDOW_DAYS`; the user input was removed). `defaultEffortHours`/`planningWindowDays` columns are kept but no longer user-editable (effort now comes from AI analysis; window is constant).
