...
// Phase 1 supports file upload (PDF/DOCX/TXT/MD) and pasted text; photo notes
// (handwriting → Gemini vision) arrive in Phase 2. Adding/deleting a note marks
// the guide/practice as stale (onNotesChanged) so the student is prompted to
// regenerate — generation is cached and won't auto-refresh.
...
const ERROR_TEXT: Record<string, string> = {
  ...
  scanned_pdf: "This PDF looks like scanned images with no selectable text. For now, paste the text (photo notes are coming soon).",
...
export function NotesSection({
  ...
  async function savePaste(title: string, text: string) {
    setBusy(true);
    setError(null);
    try {
      const res = await fetch("/api/notes", {
        ...
        body: JSON.stringify({ canvasId, title, text }),
      });
      const json = await res.json().catch(() => ({}));
      if (res.ok && json?.ok) {
        setShowPaste(false);
  ...
  return (
    <section className="card p-6">
      ...
      {showPaste && <PasteModal busy={busy} onClose={() => setShowPaste(false)} onSave={savePaste} />}
    </section>
  );
}
